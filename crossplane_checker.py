...
from crossplane.lexer import lex as lex_file
from crossplane.parser import parse as parse_file
from crossplane.builder import build as build_string, build_files
from crossplane.compat import json, input
...
def parse(filename, out, indent=None, catch=None, tb_onerror=None, ignore='', comments=False, strict=False):
...
lex(filename='bigip_nginx.conf')

parse(filename='bigip_nginx.conf', indent=2, out='bigip_nginx.json')

# build(filename='bigip_nginx.json')

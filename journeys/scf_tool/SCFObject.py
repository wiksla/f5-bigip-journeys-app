...
class SCFProperties(SCFValue, OrderedDict):
        ...
        for k in self.keys() if not order else order(self.keys()):
            if str(k).startswith("_!"):
                ...
                r += "%s%s {%s%s}\n" % (
                    " " * tab,
                    k,
                    self[k]._tostr(order=order, tab=tab + self._tabstop),
                    " " * tab,
                )
    ...
    def _as_base_type(self):
        return {
            k: self[k]._as_base_type() if isinstance(v, SCFValue) else v
...
class SCFObject:
    ...
    def __lt__(self, other):
        if not isinstance(other, SCFObject):
            raise TypeError(
                "Cannot compare %s and %s for sorting"
                % (self.__class__, other.__class__)
            )
        if str(self.fulldef).lower() < str(other.fulldef).lower():
            return True
        return False
    ...
    def folder(self):
        return (
            self.objname.split("/")[1:-1]
            if self.objname and self.objname.startswith("/")
            else None
        )
    ...
    def as_tmsh(self, mode="create", props_mode="replace-all-with"):
        return "tmsh %s %s %s" % (
            mode,
            self.fulldef,
            self.props.as_tmsh(mode=props_mode)[len(props_mode) + 1 :],
        )

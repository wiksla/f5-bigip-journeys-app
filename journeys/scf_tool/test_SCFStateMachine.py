...
class TestSingleObjThreeProps(TestCase):
    ...
    revision 13
        ...
        cls.comp[-1].props[
            "cache-path"
        ] = "/config/filestore/files_d/Common_d/trust_certificate_d/:Common:dtca-bundle.crt_32426_13"
        cls.comp[-1].props[
            "checksum"
        ] = "SHA1:1298:6b02644bf16c711fec92a63c53258d80b8d85b85"
...
class TestSingleObjThreeNests(TestCase):
    ...
    profiles {
        ...
        cls.comp[-1].objname = "/Common/APM-1-test_VS"
        cls.comp[-1].props["profiles"] = SCFProperties()
        cls.comp[-1].props["profiles"][
            "/Common/APM_connectivity_profile"
        ] = SCFProperties()
        cls.comp[-1].props["profiles"]["/Common/APM_connectivity_profile"][
            "context"
        ] = "clientside"
        cls.comp[-1].props["profiles"]["/Common/scconnect-clientssl"] = SCFProperties()
        cls.comp[-1].props["profiles"]["/Common/scconnect-clientssl"][
            "context"
        ] = "clientside"
        cls.comp[-1].props["profiles"][
            "/Common/serverssl-insecure-compatible"
...
class TestiAppSingleTableRow(TestCase):
    ...
    tables {
        ...
        cls.comp[-1].props["tables"]["data_opt__http_servers"][
            "column-names"
        ] = BracedSSVList(["name", "ip", "port"])
...
class TestAPMPolicyItem(TestCase):
    ...
    rules {
        ...
        cls.comp[-1].props["rules"][-1][
            "expression"
        ] = '"expr {[mcget {session.localdb.last.result}] == 1}"'
        cls.comp[-1].props["rules"][-1][
            "next-item"
        ] = "/Common/APM-1_access_profile_act_pool_assign"
        cls.comp[-1].props["rules"].append(SCFProperties())
        cls.comp[-1].props["rules"][-1]["caption"] = '"Locked User Out"'
        cls.comp[-1].props["rules"][-1][
            "expression"
        ] = '"expr {[mcget {session.localdb.last.result}] == 2}"'
        cls.comp[-1].props["rules"][-1][
            "next-item"
        ] = "/Common/APM-1_access_profile_end_deny"
        cls.comp[-1].props["rules"].append(SCFProperties())
...
proc iapp_get_items { args } {
    ...
    iapp_process_flags flags args
        ...
        cls.comp[-1].props[
            "_blob"
        ] = r"""proc iapp_make_safe_password { password } {
    return [string map { \' \\\\\\\' \# \\\\\\\# \\ \\\\\\\\ } $password]
}
proc iapp_get_items { args } {
...
class TestGTMServer(TestCase):
    ...
    def setup_class(cls):
        cls.text = (
            "gtm server /Common/BIAP-ERFPPS01 {\n"
            "    addresses {\n"
            "        10.43.126.55 {\n"
...
class TestGTMServerMonitorTrailingSpace(TestCase):
    ...
    def setup_class(cls):
        cls.text = (
            "gtm server /Common/C608YZADAS01B {\n"
            "    addresses {\n"
            "        10.51.2.89 {\n"
        ...
        cls.comp[-1].props["virtual-servers"]["10_51_2_89_8140"][
            "destination"
        ] = "10.51.2.89:8140"
    ...
    version 11.5.2
        ...
        cls.comp[-1].props["active-modules"] = BracedSSVList(
            [
                ...
                '"CGN, VE (LAB)|PXDKSXG-SNKNMWS|SSL, VE"',
                '"DNSSEC|HZIKNRK-YUWIGOD"',
...
class TestLTMPolicy(TestCase):
    ...
    strategy /Common/first-match
        ...
        cls.comp[-1].props["rules"]["PAC-Redirect"]["actions"]["0"][
            "http-host"
        ] = Flag()
...
class TestLTMPoolMultiMonitor(TestCase):
    ...
    def setup_class(cls):
        cls.text = (
            "ltm pool /Common/MYVODA-8080-PRD-Pool {\n"
        ...
        cls.comp[-1].props["members"]["/Common/VAAS015002:8080"][
            "address"
        ] = "172.16.33.27"
        cls.comp[-1].props["members"]["/Common/VAAS016002:8080"] = SCFProperties()
        cls.comp[-1].props["members"]["/Common/VAAS016002:8080"][
            "address"
        ] = "172.16.34.27"
        cls.comp[-1].props["members"]["/Common/VANS015002:8080"] = SCFProperties()
        cls.comp[-1].props["members"]["/Common/VANS015002:8080"][
            "address"
        ] = "172.16.209.25"
        cls.comp[-1].props["members"]["/Common/VANS016002:8080"] = SCFProperties()
        cls.comp[-1].props["members"]["/Common/VANS016002:8080"][
            "address"
        ] = "172.16.212.25"

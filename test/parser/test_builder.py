            ...
            "block": [
                {
                    ...
                    "block": [
                        ...
                        {"directive": "server_name", "args": ["default_server"]},
                        {
                            "directive": "location",
                            "args": ["/"],
                            "block": [
                                {"directive": "return", "args": ["200", "foo bar baz"]}
                            ],
                        },
                    ],
                }
    ...
    payload = [
        {
            ...
            "line": 1,
            "args": [],
            "block": [{"directive": "worker_connections", "line": 2, "args": ["1024"]}],
        },
...
def test_build_starts_with_comments():
    payload = [
        {"directive": "#", "line": 1, "args": [], "comment": " foo"},
        {"directive": "user", "line": 5, "args": ["root"]},
    ]
    built = parser.build(payload, indent=4, tabs=False)
    assert built == "# foo\nuser root;"
...
def test_build_with_quoted_unicode():
    payload = [{"directive": "env", "line": 1, "args": ["русский текст"]}]
    built = parser.build(payload, indent=4, tabs=False)
...
def test_build_multiple_comments_on_one_line():
    payload = [
        {"directive": "#", "line": 1, "args": [], "comment": "comment1"},
        {"directive": "user", "line": 2, "args": ["root"]},
        {"directive": "#", "line": 2, "args": [], "comment": "comment2"},
        {"directive": "#", "line": 2, "args": [], "comment": "comment3"},
    ]
    built = parser.build(payload, indent=4, tabs=False)
...
def test_build_files_with_missing_status_and_errors(tmpdir):
    ...
    payload = {
        "config": [
            {
                "file": "nginx.conf",
                "parsed": [{"directive": "user", "line": 1, "args": ["nginx"]}],
            }
        ]
    }
        ...
        "config": [
            {
                ...
                "status": "ok",
                "errors": [],
                "parsed": [{"directive": "user", "line": 1, "args": [u"測試"]}],
            }
        ],
    }
    parser.builder.build_files(payload, dirname=tmpdir.strpath)
